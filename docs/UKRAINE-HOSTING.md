...
Саме через панель: `crontab -` по SSH на цьому хостингу заблокований
CloudLinux (падає з PermissionError).

Чекати найближчого запуску не обовʼязково: у панелі біля завдання є кнопка
«Виконати зараз» — зручно, коли треба викотити терміново.
...
| зовсім порожній вивід | Викочена версія вже збігається з `main` — робити нічого. Примусово перевикотити: `rm tmp/deployed.sha && bash scripts/deploy.sh`. |
